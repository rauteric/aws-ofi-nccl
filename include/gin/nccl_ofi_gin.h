...
public:
	...
	nccl_ofi_device_copy &get_device_copy_ctx() { return *copy_ctx; }
...
/**
 * The listen communicator which implements GIN API's listen() and connect() functionality
 */
...
private:
	...
	int iput_signal_deliver_all(uint64_t peer_rank);
...
public:
	 ...
	 */
	int regMrSymDmaBuf(nccl_ofi_mr_ckey_ref ckey, void *data_ptr,
	 ...
	 * @return: 0 on success, non-zero on failure
	 */
	int iputSignal(uint64_t srcOff, gin_sym_mr_handle* srcMhandle,
	 ...
	 * @return: 0 on success, non-zero on failure
	 */
	int handle_signal_metadata_completion(fi_addr_t src_addr, uint16_t rail_id,
