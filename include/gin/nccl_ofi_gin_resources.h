...
public:
	...
	nccl_ofi_gin_ep_rail_t &get_rail(uint16_t rail_id) { return rails[rail_id]; }
...
{
public:
	nccl_ofi_gin_mr_handle_t(nccl_net_ofi_domain_t &domain_arg, uint16_t num_rails, uint64_t mr_key_arg) :
...
private:
	 ...
	 */
	struct write_ack_buffer
	{
		void *addr;
		nccl_ofi_gin_mr_handle_t *mr_handle;

		write_ack_buffer(nccl_ofi_gin_ep_t &ep);
		~write_ack_buffer();
	};

	write_ack_buffer write_ack_buffer;
