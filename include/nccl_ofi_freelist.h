 ...
 *
 * The mr_handle field of the reginfo_t structure will contain the handle
 * returned from regmr_fn() being called for the allocation block and the
 * base_offset field will contain the offset (in bytes) from the start of the
 * memory registartion to the start of the returned freelist entry, allowing for
 * use with providers that require 0-based registration accesses.
 */
 ...
 * reginfo_t structure will contain valid information for the mr_handle and
 * base_offset fields.  The caller should not write into the bytes covered by
 * the reginfo_t structure.
 */
