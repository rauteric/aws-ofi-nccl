	...
	copy_ctx(new nccl_ofi_gdrcopy_ctx())
{
	if (copy_ctx->forced_pcie_copy() == false) {
		throw std::runtime_error("GDRCopy does not support forced PCIe copy (GDRCopy 2.5+ required)");
	}
}
		...
		metadata_fl(nullptr, &freelist_deleter)
{
	...
	int ret = nccl_ofi_freelist_init_mr
		(sizeof(nccl_net_ofi_gin_signal_metadata_msg_t), 16, 16, 0, nullptr, nullptr,
		 ep.freelist_regmr_fn, ep.freelist_deregmr_fn, &ep,
		 1, &metadata_fl_ptr);
	if (ret != 0) {
				      ...
				      int nranks, int rank, nccl_ofi_gin_comm** gin_comm_out)
{
	...
	nccl_ofi_gin_comm *gin_comm = new nccl_ofi_gin_comm(*resources,
							    ep->get_domain().get_device()->dev_id,
							    rank, nranks, s_comm, r_comm,
							    gin_ctx->get_device_copy_ctx());
...
int nccl_ofi_gin_comm::writedata_ack(nccl_ofi_gin_comm &gin_comm, unsigned int peer_rank,
...
int nccl_ofi_gin_comm::regMrSymDmaBuf(nccl_ofi_mr_ckey_ref ckey, void *data_ptr,
...
int nccl_ofi_gin_comm::iputSignal(uint64_t srcOff, gin_sym_mr_handle* srcMhandle,
				  size_t size, uint64_t dstOff, gin_sym_mr_handle* dstMhandle,
				  uint32_t dst_rank, uint64_t signalOff, gin_sym_mr_handle* signalMhandle,
				  ...
				  nccl_net_ofi_gin_iputsignal_req_t** request)
{
	...
	if (signalOp != 0) {
		...
		metadata_send->signal_base_address = (signalMhandle ? signalMhandle->remote_mr[dst_rank].address
							: 0);
...
static inline uint64_t get_peer_rank(fi_addr_t src_addr,
				     std::unordered_map<fi_addr_t, uint64_t> &rank_map)
{
...
int nccl_ofi_gin_comm::do_gin_signal(const nccl_net_ofi_gin_signal_metadata_msg_t &metadata)
{
	...
	if (mr_handle->type == NCCL_PTR_CUDA) {
		...
		int ret = this->copy_ctx.copy_from_device
...
int nccl_ofi_gin_comm::iput_signal_recv_req_completion(unsigned int peer_rank, uint64_t map_key,
...
int nccl_ofi_gin_comm::handle_signal_metadata_completion(fi_addr_t src_addr, uint16_t rail_id,
							 const nccl_net_ofi_gin_signal_metadata_msg_t *metadata_msg)
{
	...
	uint64_t peer_rank = get_peer_rank(src_addr, rank_map[rail_id]);
	...
	if (it == outstanding_iput_signal_recv_reqs.end()) {
		auto *req = resources.get_req_from_pool
