...
static ncclResult_t nccl_ofi_gin_init(void **ctx, uint64_t commId, ncclDebugLogger_t logFunction)
{
	...
	try {
		*ctx = new nccl_ofi_gin_ctx();
		...
		NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Failed to allocate GIN ctx; GDRCopy is likely not available");
		*ctx = nullptr;
					 ...
					 void* listenComm, void** collComm)
{
	...
	try {
		ret = gin_l_comm->connect(gin_ctx, gin_handles,
					nranks, rank,
					    ...
					    uint64_t signalValue, uint32_t signalOp, void** request)
{
	...
	int ret = gin_comm->iputSignal(srcOff, src_mr_handle, size, dstOff, dst_mr_handle,
				       rank, signalOff, signal_mr_handle, signalValue, signalOp,
				       &req);
	if (ret != 0) {
		return nccl_net_ofi_retval_translate(ret);
	}
