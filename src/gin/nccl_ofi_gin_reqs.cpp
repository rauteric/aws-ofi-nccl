						 ...
						 uint16_t rail_id_arg)
{
	...
	if (cq_entry->flags & FI_REMOTE_WRITE) {
		...
		ret = gin_comm.handle_signal_write_completion
			(src_addr, rail_id_arg, msg_seq_num, total_segms, len);
		if (ret != 0) {
			return ret;
		}
	} else {
		...
		ret = gin_comm.handle_signal_metadata_completion
			(src_addr, rail_id_arg, msg);
		if (ret != 0) {
...
int nccl_net_ofi_gin_metadata_send_req_t::post()
{
	...
	ssize_t rc = fi_send(ep, metadata_elem->ptr, sizeof(nccl_net_ofi_gin_signal_metadata_msg_t),
			fi_mr_desc(metadata_handle->get_mr(rail_id)),
