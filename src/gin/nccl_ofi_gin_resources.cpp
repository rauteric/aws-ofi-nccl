					       ...
					       uint16_t rail_id)
{
	...
	for (uint64_t comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		nccl_net_ofi_gin_context *ctx = cpp_container_of
			(static_cast<struct fi_context2 *>(op_ctx),
...
int nccl_ofi_gin_ep_t::reg_mr(nccl_ofi_mr_ckey_ref ckey, int type, nccl_ofi_gin_mr_handle_t **mhandle)
{
	...
	*mhandle = NULL;
	...
	auto ret_handle = std::make_unique<nccl_ofi_gin_mr_handle_t>(this->domain, num_rails, mr_key);
 ...
 */
static inline void get_gin_hints(struct fi_info &hints)
{
	hints.caps = 0;
...
nccl_ofi_gin_resources::write_ack_buffer::write_ack_buffer(nccl_ofi_gin_ep_t &ep)
{
...
nccl_ofi_gin_resources::write_ack_buffer::~write_ack_buffer()
{
	int ret = 0;
	  ...
	  rx_buff_fl(nullptr, &freelist_deleter)
{
	...
	int ret = nccl_ofi_freelist_init_mr
		...
		nullptr, nullptr, gin_ep.freelist_regmr_fn, gin_ep.freelist_deregmr_fn, &gin_ep,
		1, &rx_buff_fl_tmp);
	if (ret != 0) {
