 ...
 */
static inline nccl_net_ofi_rdma_ctrl_msg_t *send_ctrl_get_msg
 ...
 */
static inline nccl_net_ofi_rdma_close_msg_t *send_close_get_msg
				     ...
				     nccl_net_ofi_rdma_req_t *bounce_req, bool eager)
{
	...
	nccl_ofi_rdma_msg_type_t msg_type =
		eager ? NCCL_OFI_RDMA_MSG_EAGER : get_bounce_ctrl_msg(bounce_data)->type;
					      ...
					      bool dec_inflight_reqs)
{
	...
	if (send_ctrl_data->ctrl_fl_elem) {
		nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
		nccl_ofi_freelist_entry_free_mr(r_comm->ctrl_buff_fl, send_ctrl_data->ctrl_fl_elem);
					      ...
					      bool dec_inflight_reqs)
{
	...
	if (send_close_data->ctrl_fl_elem) {
		nccl_net_ofi_rdma_recv_comm_t *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
		nccl_ofi_freelist_entry_free_mr(r_comm->ctrl_buff_fl, send_close_data->ctrl_fl_elem);
				...
				nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle)
{
	 ...
	 */
	send_ctrl_data->ctrl_fl_elem = nccl_ofi_freelist_entry_alloc_mr
					(r_comm->ctrl_buff_fl);
...
static inline int recv_comm_insert_send_close_req(nccl_net_ofi_rdma_recv_comm_t *r_comm)
{
	 ...
	 */
	send_close_data->ctrl_fl_elem = nccl_ofi_freelist_entry_alloc_mr
		(r_comm->ctrl_buff_fl);
	if (send_close_data->ctrl_fl_elem == NULL) {
		NCCL_OFI_WARN("Call to nccl_ofi_freelist_entry_alloc_mr failed");
		send_close_req->free(send_close_req, false);
		return -ENOMEM;
	}
			      ...
			      nccl_net_ofi_ep_rail_t *ep_rail)
{
	...
	freelist_regmr_fn_handle_t *fl_mr_handle =
		(freelist_regmr_fn_handle_t *)bounce_fl_elem->reginfo.mr_handle;
	...
	req->state = NCCL_OFI_RDMA_REQ_CREATED;
	ssize_t rc =
		fi_recv(ep_rail->ofi_ep, bounce_fl_elem->ptr, bounce_data->buff_len, desc, FI_ADDR_UNSPEC, req);
	if ((rc != 0) && (rc != -FI_EAGAIN)) {
...
static int post_eager_copy(nccl_net_ofi_rdma_req_t *req)
{
	...
	/* Unpack mr_handle */
	freelist_regmr_fn_handle_t *fl_handle =
		(freelist_regmr_fn_handle_t *)bounce_data->bounce_fl_elem->reginfo.mr_handle;
...
error:
	if (plugin) {
		if (s_comm_cleanup_list) {
			nccl_ofi_deque_finalize(s_comm_cleanup_list);
			s_comm_cleanup_list = NULL;
		}
		if (r_comm_cleanup_list) {
			nccl_ofi_deque_finalize(r_comm_cleanup_list);
			r_comm_cleanup_list = NULL;
		}
