 ...
 */
static inline int handle_rx_buff_recv(int rail_id, struct fi_cq_data_entry *cq_entry,
...
{
	int ret = 0;
	auto cq_entry = static_cast<fi_cq_data_entry *>(cq_entry_base);
	...
	if (comp_flags & FI_SEND) {
		...
		/* Receive completions */
		ret = handle_rx_buff_recv(rail_id, cq_entry, req,
					   ...
					   int rail_id)
{
	...
	for (uint64_t comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		ret = ctx->handle_cq_entry(ctx, static_cast<struct fi_cq_entry *>(&cq_entry[comp_idx]),
					   rail_id);
		if (ret != 0) {
...
static inline void *rdma_req_get_ofi_context(nccl_net_ofi_rdma_req_t *req, int rail_id)
{
