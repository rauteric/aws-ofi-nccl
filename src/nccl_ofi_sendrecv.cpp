...
{
	(void) rail_id;
	auto cq_entry = static_cast<struct fi_cq_tagged_entry *>(cq_entry_base);
						...
						int rail_id)
{
	...
	assert(((struct fi_cq_tagged_entry *)cq_entry_base)->flags & FI_RECV);
					       ...
					       uint64_t num_cqes)
{
	...
	for (uint64_t comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		ret = ctx->handle_cq_entry(ctx, (struct fi_cq_entry *)&cq_entry[comp_idx], 0);
		if (ret != 0) {
