				     ...
				     nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	if (OFI_UNLIKELY(!domain->base.domain_active)) {
		NCCL_OFI_WARN("Called mr_base_reg on request with inactive domain");
		return -EINVAL;
	}
				       ...
				       nccl_net_ofi_mr_handle_t *mhandle)
{
	...
	if (!domain->base.domain_active) {
		NCCL_OFI_WARN("Called dereg_mr on inactive domain");
		return -EINVAL;
	}
	...
	(nccl_net_ofi_sendrecv_recv_comm_t *r_comm)
{
	...
	char *local_address = sendrecv_get_local_address(r_comm->local_ep);
				       ...
				       nccl_net_ofi_recv_comm_t **recv_comm)
{
	...
	*recv_comm = NULL;
	...
	case COMM_CONN_RESP_REQ_PENDING:
		...
		if (ret <= 0) {
			/* Error case, or receiver not ready */
			return ret;
		}
				     ...
				     int trafficClass)
{
	...
	*send_comm = NULL;
	...
	case COMM_CREATE_START:
		...
		/* Build send_comm */
		ret = sendrecv_send_comm_create(handle, ep, &conn_info, &s_comm);
	...
	case COMM_SEND_CONN:
		...
		s_comm->connector = domain->cm->connect(*handle, &conn_info);
	...
	case COMM_CONN_REQ_PENDING:
		...
		assert(s_comm->connector != nullptr);
		...
		/* Populate info from connect response message */
		{
			...
			ret = sendrecv_send_comm_process_conn_resp(s_comm, ep, dev_id,
								   *conn_resp_msg);
			if (ret != 0) {
				free(s_comm);
				return ret;
